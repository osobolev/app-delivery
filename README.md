...
- `options.bat`/`options.sh` - скрипт задания настроек приложения
...
- `jre`/`jre64`/`jre.win`/`jre64.win`/`jre.lin`/`jre64.lin` - архивы среды исполнения Java для обновления Java на клиенте. Имя архива должно иметь формат `java[^-]*-<version>.zip`, где `<version>` - версия Java из свойства `java.vm.version`.
...
- `java.bits`: версия Java - 32 или 64 бита
...
- `name`: имя профиля для отображения пользователю
